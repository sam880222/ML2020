...
def predict(latents):
    ...
    transformer = KernelPCA(n_components=512, kernel='rbf', random_state = 0, n_jobs=-1)
    kpca = transformer.fit_transform(latents)
    print('First Reduction Shape:', kpca.shape)

    transformer2 = PCA(n_components=400, random_state=0)
    pca = transformer2.fit_transform(kpca)

    transformer3 = PCA(n_components=200, random_state=0)
    pca = transformer3.fit_transform(pca)

    # # Second Dimesnion Reduction
    X_embedded = TSNE(n_components=2, random_state = 0, n_jobs = -1).fit_transform(pca)
